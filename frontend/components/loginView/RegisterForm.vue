<template>
  <div class="space-y-6">
    <div class="text-center">
      <p class="mb-3 text-xs font-semibold uppercase tracking-[0.35em] text-primary">{{ t('login.modes.register.eyebrow') }}</p>
      <h2 class="text-3xl font-semibold tracking-tight text-foreground">{{ t('login.modes.register.title') }}</h2>
      <p class="mt-3 text-sm leading-6 text-muted-foreground">{{ t('login.modes.register.description') }}</p>
    </div>

    <form class="space-y-5" @submit.prevent="emit('submit')">
      <div class="space-y-2">
        <label for="register-full-name" class="block text-sm font-medium text-foreground">{{ t('login.fullNameLabel') }}</label>
        <input
          id="register-full-name"
          v-model="fullNameModel"
          type="text"
          :placeholder="t('login.fullNamePlaceholder')"
          class="h-12 w-full rounded-2xl border border-border/80 bg-card px-4 text-sm text-foreground outline-none transition focus:border-primary/50 focus:ring-4 focus:ring-primary/10"
      ...
      <div class="space-y-2">
        <label for="register-email" class="block text-sm font-medium text-foreground">{{ t('login.emailLabel') }}</label>
        <div class="relative">
      ...
      <div class="space-y-2">
        <label for="register-password" class="block text-sm font-medium text-foreground">{{ t('login.passwordLabel') }}</label>
        <div class="relative">
          <input
            ...
            class="h-12 w-full rounded-2xl border border-border/80 bg-card px-4 pr-12 text-sm text-foreground outline-none transition focus:border-primary/50 focus:ring-4 focus:ring-primary/10"
            @focus="emit('focus-change', true)"
            @blur="emit('focus-change', false)"
          >
          <button
            type="button"
            class="absolute inset-y-0 right-0 flex items-center pr-4 text-muted-foreground transition hover:text-foreground"
      ...
      <div class="space-y-2">
        <label for="register-confirm-password" class="block text-sm font-medium text-foreground">{{ t('login.confirmPasswordLabel') }}</label>
        <input
