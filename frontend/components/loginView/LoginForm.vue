<template>
  <div class="space-y-6">
    <div class="text-center">
      <p class="mb-3 text-xs font-semibold uppercase tracking-[0.35em] text-primary">{{ t('login.modes.login.eyebrow') }}</p>
      <h2 class="text-3xl font-semibold tracking-tight text-foreground">{{ t('login.modes.login.title') }}</h2>
    </div>
    ...
    <form class="space-y-5" @submit.prevent="emit('submit')">
      ...
      <div class="space-y-2">
        <label for="login-password" class="block text-sm font-medium text-foreground">{{ t('login.passwordLabel') }}</label>
        <div class="relative">
          <input
            id="login-password"
      ...
      <div class="flex items-center justify-between gap-4">
        ...
        <button type="button" class="text-sm font-medium text-primary transition hover:text-primary-strong" @click="emit('navigate-forgot-password')">
    ...
    <div class="my-6 flex items-center gap-4 text-xs uppercase tracking-[0.25em] text-muted-foreground">
      <div class="h-px flex-1 bg-border" />
      <span>{{ t('login.orContinueWith') }}</span>
      <div class="h-px flex-1 bg-border" />
    </div>

    <button type="button" class="flex h-12 w-full items-center justify-center gap-3 rounded-2xl border border-border/80 bg-card text-sm font-medium text-foreground transition hover:border-border-strong hover:bg-panel/70">
    ...
    <p class="text-center text-sm text-muted-foreground">
      {{ t('login.noAccount') }}
      <button type="button" class="ml-1 font-medium text-foreground transition hover:text-primary" @click="emit('navigate-register')">
        {{ t('login.signUp') }}
      </button>
    </p>
  </div>
